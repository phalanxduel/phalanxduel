...
- `rtk pnpm dev:server`: Starts the Fastify/WebSocket server.
- `rtk pnpm dev:client`: Starts the Vite development server.
- `rtk pnpm dev:admin`: Starts the Admin UI.
...
Manual commands:
- `rtk bash bin/maint/with-dev-postgres.sh pnpm --filter @phalanxduel/server db:migrate`: Run migrations against the project development database.
- `rtk pnpm admin:seed-dev`: Seed a development administrator account (`mike@phalanxduel.com` / `adminadmin`).
...
rtk docker-compose up -d pghero
```
...
| Data + admin tools | Project development `DATABASE_URL` in the launching shell |
...
| `match_embed` / `match_find_similar` | Project development `DATABASE_URL` + `OPENAI_API_KEY` |
...
Set these in the shell that launches Claude Code for full local access. Use the
project development database role and database, not an ambient personal or
system Postgres URL:

```bash
export DATABASE_URL="postgresql://phalanx_dev:phx_dev_local@localhost:5432/phalanxduel_development"
...
2.  Make your changes.
3.  Run `rtk pnpm verify:quick` to check linting and types.
4.  Run focused tests for the package you modified (e.g., `rtk pnpm test:run:engine`).
