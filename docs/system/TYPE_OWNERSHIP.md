...
Tooling boundary:
- Runtime packages (`client/src`, `server/src`, `engine/src`, `shared/src`)
  should not import from `bin/`, `scripts/`, or test-only modules.
- Tooling-shared types are allowed, but they belong in tooling-only modules and
  must not leak into production runtime dependency paths.
...
- If two non-test files need the same local shape, extract it once instead of
  copying it.
- Keep a hard firewall between production runtime code and dev/test/tooling
  code. Sharing a type through a tooling module is fine for scripts, but
  production packages should not load Playwright, Vitest, Knip, dependency
  analysis helpers, or similar dev-only surfaces.
