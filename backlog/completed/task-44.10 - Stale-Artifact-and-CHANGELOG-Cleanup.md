...
Several housekeeping issues: (1) `CHANGELOG.md` version `[0.2.4-rev.8]` has an empty "### Fixed" section that runs into `[0.3.0-rev.6]` content, creating parsing ambiguity. (2) Completed planning documents in `docs/archive/` (e.g., the OTel migration plan from 2026-03-10) may mislead agents into re-executing completed work. (3) `docs/archive/doc-1 - GLOSSARY.md.md` has a double `.md` extension (also addressed in TASK-44.6).

**Concern sources:**
- **Claude Code/Opus 4.6**: Flagged CHANGELOG empty "Fixed" section as a formatting defect creating version-boundary ambiguity. Noted `docs/archive/PLAN - 2026-03-10 - otel-native-hybrid-plan.md` as potentially archivable since OTel migration appears complete. Flagged `doc-1 - GLOSSARY.md.md` double extension.
- **Gordon**: Recommended archiving completed planning docs to `backlog/archive/plans/` and keeping only active plans in `docs/archive/`. Flagged risk of new contributors reading completed plans as current roadmap.
...
- [x] #2 Completed planning documents in `docs/archive/` are reviewed and either archived to `backlog/completed/docs/` or marked with completion status.
- [x] #3 `docs/archive/doc-1 - GLOSSARY.md.md` double `.md` extension is fixed.
<!-- AC:END -->
...
2. Review `docs/archive/PLAN - 2026-03-10 - otel-native-hybrid-plan.md` — if OTel migration is complete, move to `backlog/completed/docs/`.
3. Review other plans in `docs/archive/` for completion status.
...
- [x] #3 **Accessibility (DoD §6)**: Contributors reading `docs/archive/` see only active or in-progress plans, not completed work.
<!-- DOD:END -->
