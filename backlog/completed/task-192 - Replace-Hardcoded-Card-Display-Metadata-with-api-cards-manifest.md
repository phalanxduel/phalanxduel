...
Added client/src/manifest.ts with module-level cache for /api/cards/manifest. Updated isFace() in cards.ts to use manifest type data when available, falling back to card.type. Wired fetchCardsManifest() call in main.ts init(). Added 10 isolated vitest tests in client/tests/manifest.test.ts covering cache lifecycle, idempotency, error fallbacks, and isFace integration. All 205 client tests pass.
