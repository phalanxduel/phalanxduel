...
func _ready() -> void:
	...
	refresh_btn.set_meta("data_test_id", "spectator-lobby-refresh")
	...
	_status_label.text = "SYNCHRONIZING_DIRECT_STREAMS..."
	...
	# Connect to spectator specific endpoint (reusing repository for now)
	# Need to implement /api/spectator/matches fetch
	# Temporarily use lobby matches to verify UI
...
func _build_match_item(data: Dictionary) -> PanelContainer:
	...
	panel.set_meta("data_test_id", "spectator-lobby-match-row")
	...
	creator.text = str(data.get("creatorName", "UNKNOWN")) + " vs " + str(data.get("opponentName", "UNKNOWN"))
	creator.add_theme_font_size_override("font_size", 16)
	v_info.add_child(creator)
	
	var details := Label.new()
	var match_id: String = str(data.get("matchId", ""))
	details.text = "ID: %s | SPECTATORS: %d" % [match_id.substr(0, 8), int(data.get("spectatorCount", 0))]
	...
	watch_btn.custom_minimum_size = Vector2(120, 0)
	watch_btn.set_meta("data_test_id", "spectator-lobby-watch-btn")
	watch_btn.pressed.connect(func(): emit_signal("match_selected", match_id))
