...
function BattlefieldCell({
  ...
  const onClick = () => {
    const { playerIndex, selectedAttacker, selectedDeployCard } = state;
    if (isTargetable && playerIndex !== null && selectedAttacker)
      sendAction(state, {
        type: 'attack',
        playerIndex,
        attackingColumn: selectedAttacker.col,
        defendingColumn: col,
    ...
    } else if (isDeployable && playerIndex !== null && selectedDeployCard)
      sendAction(state, {
        type: 'deploy',
        playerIndex,
        column: col,
        cardId: selectedDeployCard,
        timestamp: new Date().toISOString(),
      });
    else if (isReinforceable && playerIndex !== null && selectedDeployCard)
      sendAction(state, {
        type: 'reinforce',
        playerIndex,
        cardId: selectedDeployCard,
...
function PhxSidebar({ gs, state }: { gs: GameState; state: GameScreenState }) {
  ...
  const hasAttack = (gs.transactionLog ?? []).some((e) => e.details.type === 'attack');
    ...
    return (
      <div class="phx-game-layout phx-spectator-waiting" data-testid="spectator-waiting">
        <div class="phx-spectator-waiting-card">
          ...
          <h2>
            Waiting for delayed frame<span class="phx-waiting-dots">…</span>
          </h2>
          <p>The live match is connected. Replay-safe frames appear after the three-turn delay.</p>
          <p class="phx-spectator-waiting-hint">
            Stay on this page; the board will populate automatically.
          </p>
        </div>
      </div>
    );
