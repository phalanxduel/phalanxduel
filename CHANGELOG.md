...
- **Tournament Operations Suite**: Dedicated tools for live tournament management, including `bin/phx-tournament-top` for network monitoring, `bin/phx-tournament-check` with local firewall inspection (`socketfilterfw`/`pf`), and comprehensive tournament runbooks (`docs/ops/TOURNAMENT_RUNBOOK.md`).
- **Live Terminal Monitor**: Built `bin/phx-top` (`pnpm top`) providing real-time game analytics, memory, database stats, and active duel telemetry.
- **Cinematic Repository Visualizer**: `bin/phx-gource` runner generating multi-repository history visualizer across `game`, `game-swiftui`, `site`, `wiki`, and `monitor`.
- **Full Stack Architecture & Operational Blueprint**: Authoritative documentation (`docs/architecture/FULL_STACK.md`) covering service topologies, ports, protocols, and database isolation.
...
- **Repository Hygiene**: Excised legacy archives, experiments, and knowledge-graph dumps under local gitignored `artifacts/` (-168k lines / 60MB), preserving a clean root directory.
- **Vite & Client Build Tuning**: Optimized Rolldown code splitting, eliminated dynamic import warnings, and tuned chunk size warning threshold.
...
- **Match Replay & Historical Rewatch**: Repaired `fast-json-stringify` tuple normalization in `server/src/utils/openapi.ts` and enabled `additionalProperties: true` on action log route, restoring 100% (20/20) successful replay reconstruction for all matches.
- **Spectator UI & Replay Scrubber**: Cleaned DOM resets between turns in `RewatchGameFrame` and decoupled history errors from active match stream errors in the lobby.
...
[1.5.0]: https://github.com/phalanxduel/phalanxduel/compare/v1.3.0...v1.5.0
...
[1.0.0]: https://github.com/phalanxduel/phalanxduel/compare/v0.5.0-rev.5...v1.0.0
[0.5.0-rev.5]: https://github.com/phalanxduel/phalanxduel/releases/tag/v0.5.0-rev.5
